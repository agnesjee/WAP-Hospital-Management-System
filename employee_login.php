...
unset($_SESSION['account']); //Logout current user
  ...
  if ( empty($_POST['emID']) || $_POST['emID'] == " ") {
    $_SESSION['error'] = "Please enter Employee ID.";
    ...
    $sql = "SELECT * FROM employee WHERE emID=:emID AND emPassword=:emPassword AND (emPos='Doctor' OR emPos='Nurse') ";
    ...
    if($stmt->rowCount() > 0) {
    	if( $_POST['emID'] == $row['emID']) {
    		...
    		$_SESSION['error'] = "Incorrect Employee ID.";
    		header("Location: employee_login.php");
    		return;
    	}
    } else {
...
<div class="header">
  ...
  <a class="active" href="employee_login.php">Employee</a>
...
<form method="post">
  ...
  <div class="container">
    <label for="emID"><b>Employee ID</b></label>
    <input type="text" id="emID"  placeholder="Enter Employee ID" name="emID" required/>
